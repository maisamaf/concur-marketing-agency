...
const StatisticsCard = ({ icon, title, href, className, ...props }) => {
  return (
    <div className={`flex flex-col gap-5 ${className}`}>
      <div className="grid p-3 text-center bg-white border-2 border-gray-200 place-items-center max-w-max rounded-xl">
          ...
          <Link href={href}>
            <a className="mt-6 text-blue-600 transition duration-500 transform max-w-max group ">
              ...
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="inline-block w-4 h-4 ml-1 group-hover:translate-x-1"
              >
                <path
                  strokeLinecap="round"
